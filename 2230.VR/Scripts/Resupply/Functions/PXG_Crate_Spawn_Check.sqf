...
_supplyText = lbText [451502, _indexSupply];
...
if (count _nearVehicles > 0) then {
	...
	switch(_supplyData) do {
		...
		default {		
			if (((_vehicle getVariable "ace_cargo_space") - 1) >= 0) then {
				[_supplyText, _supplyData , 1, _vehicle, _spawnPosition] call compile preprocessFileLineNumbers "Scripts\Resupply\Functions\PXG_Crate_Spawn_VehicleLoad.sqf";
	...
	default {
		private _crate = createVehicle[_supplyData, _spawnPosition, [], 0, "CAN_COLLIDE"];
